...
int main() {
    int chonChucNang;
    srand(time(NULL)); // Khởi tạo bộ sinh số ngẫu nhiên cho game
    ...
    do {
        ...
        if (scanf("%d", &chonChucNang) != 1) {
            ...
            while(getchar() != '\n'); // Xóa bộ nhớ đệm để tránh lặp vô tận
            continue;
        }
...
void doiTien() {
    ...
    int soTien;
    printf("\nNhap so tien muon doi (don vi VND): "); scanf("%d", &soTien);
    for (int i = 0; i < 9; i++) {
...
void laiSuatVay() {
    double soTien, laiSuat = 0.05, goc, lai;
...
void sapXepSinhVien() {
    ...
    for (int i = 0; i < n; i++) {
        printf("Ten SV %d: ", i + 1);
        scanf(" %[^\n]", sv[i].ten); // Dấu cách trước % để tránh trôi lệnh
...
void tinhPhanSo() {
    ...
    else printf("Khong the thuc hien phep chia do tu so phan so thu 2 bang 0\n");
}